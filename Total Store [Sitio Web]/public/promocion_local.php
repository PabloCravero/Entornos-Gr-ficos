...
if ($codUsuario && $tipoUsuario == 'Cliente') {
  $busca_promociones .= " AND estadoPromo = 'aprobada' AND categoriaCliente = (SELECT categoriaCliente FROM usuarios WHERE codUsuario = $codUsuario)";
}
        ...
        if (mysqli_num_rows($resultado) > 0) {
          ...
          if ($total_pages > 1) {
            ...
            for ($i = 1; $i <= $total_pages; $i++) {
              echo '<a href="promocion_local.php?codLocal=' . $codLocal . '&page=' . $i . '"';
              if ($i === $page) {
                echo ' class="active"';
              }
              echo '>' . $i . '</a>';
